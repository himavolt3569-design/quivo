...
const KINDS: KindRow[] = [
  {
    ...
    label: "POS sale closed",
    description: "Each completed sale at the till.",
  },
  {
    id: "order.placed",
    label: "New storefront order",
    description: "Someone placed an online order with your shop.",
  },
  {
    id: "order.status_changed",
    label: "Order status updates",
    description: "Customer-facing reminders as orders progress.",
  },
  {
    id: "refund.completed",
    label: "Refund processed",
    description: "A refund was issued and stock restored.",
  },
  {
    id: "low_stock.detected",
    label: "Low-stock digest",
    description: "Daily list of SKUs at/below their threshold.",
  },
  {
    id: "kyc.stage_due",
    label: "KYC compliance reminder",
    description: "Grace, warning, and deadline emails.",
  },
];

const CHANNELS: Array<{
  id: keyof ChannelPrefs;
  label: string;
  icon: typeof Bell;
}> = [
    ...
    <div className="space-y-6 max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
      ...
      <div className="bg-white rounded-2xl border border-[#2E3344]/8 shadow-sm p-5">
        ...
        <p className="text-xs text-[#746E73] mb-3">
          Subscribes the current browser so push notifications can reach you
          even when Quivo isn&apos;t open.
        </p>
        <PushSubscribeButton />
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-[#f8f8f7]">
              <tr>
                <th className="px-4 py-3 text-left text-[10px] font-black uppercase tracking-widest text-[#746E73] w-1/2">
                  Kind
                </th>
                {CHANNELS.map((c) => (
                  <th
                    key={c.id}
                    className="px-4 py-3 text-center text-[10px] font-black uppercase tracking-widest text-[#746E73]"
                  >
                ...
                <tr key={k.id}>
                  ...
                  {CHANNELS.map((c) => {
                    ...
                    return (
                      ...
                      >
                        <label
                          className={`inline-flex items-center justify-center ${disabled ? "opacity-30 cursor-not-allowed" : "cursor-pointer"}`}
                        >
                          <input
                            type="checkbox"
                            disabled={disabled}
