            ...
            setShifts((prev) => {
              ...
              if (idx === -1) {
                ...
                const inserted: StaffShiftRow = {
                  ...
                  scheduled_start:
                    row.scheduled_start ?? new Date().toISOString(),
      ...
      setShifts((prev) =>
        prev.map((s) =>
          s.id === shiftId
            ? {
                ...s,
                clocked_in_at: res.at ?? new Date().toISOString(),
                status: "in_progress",
              }
            : s,
        ),
      ...
      setShifts((prev) =>
        prev.map((s) =>
          s.id === shiftId
            ? {
                ...s,
                clocked_out_at: res.at ?? new Date().toISOString(),
                status: "completed",
              }
            : s,
        ),
    ...
    <div className="container max-w-2xl mx-auto px-4 sm:px-6 py-6 sm:py-10 space-y-6 animate-in fade-in duration-500">
      <header>
        ...
        <p className="text-sm text-[#746E73] mt-1">
          Clock in and out of your shifts here.
        </p>
      </header>
      ...
      {partitions.active ? (
        <section className="rounded-[2rem] p-6 bg-gradient-to-br from-[#41A560] to-[#2c8b48] text-white shadow-xl">
          <div className="flex items-center gap-2 text-xs font-black uppercase tracking-widest opacity-80">
            <span className="h-2 w-2 rounded-full bg-white animate-pulse" /> On
            the clock
          </div>
          <p className="mt-3 text-sm font-medium opacity-90 flex items-center gap-2">
            <Store className="h-4 w-4" />{" "}
            {partitions.active.shop_name ?? "Shop"}
          </p>
          <p className="text-3xl font-black mt-1">
            {fmtTime(partitions.active.scheduled_start)} –{" "}
            {fmtTime(partitions.active.scheduled_end)}
          </p>
          <p className="text-xs opacity-80 mt-1">
            {fmtDateLine(partitions.active.scheduled_start)}
          </p>
          {partitions.active.clocked_in_at && (
            <p className="text-[11px] mt-2 opacity-75">
              Clocked in at {fmtTime(partitions.active.clocked_in_at)}
            </p>
          )}
          {partitions.active.notes && (
            <p className="text-xs mt-3 italic opacity-85">
              &ldquo;{partitions.active.notes}&rdquo;
            </p>
          )}
          <button
            type="button"
        ...
        <section className="rounded-[2rem] p-6 bg-white border border-[#2E3344]/8 text-center">
          ...
          <p className="text-xs text-[#746E73] mt-1">
            {partitions.today.length > 0
              ? "You have a shift scheduled for today."
              : "Check back when your next shift starts."}
          </p>
        </section>
      )}
      ...
      {partitions.today.length > 0 && (
        <section>
          <h2 className="text-[11px] font-black uppercase tracking-widest text-[#8D5132] mb-3 ml-1">
            Today
          </h2>
