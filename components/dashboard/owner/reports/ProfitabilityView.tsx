...
import { ChevronLeft, Download, TrendingUp, ArrowUpDown, AlertCircle } from "lucide-react";
import { toast } from "sonner";
import { getProductProfitability, type ProfitabilityRow } from "@/app/actions/reports";
...
export function ProfitabilityView({ shopId, shopName }: Props) {
  ...
  const exportCsv = () => {
    ...
    const out: (string | number)[][] = [
      ["Product", "Units", "Revenue", "COGS", "Gross margin", "Margin %", "Current price"],
      ...sorted.map((r) => [r.name, r.units, r.revenue.toFixed(2), r.cogs.toFixed(2), r.gross_margin.toFixed(2), r.margin_pct, r.current_price.toFixed(2)]),
      [],
      ["TOTAL", "", totals.revenue.toFixed(2), totals.cogs.toFixed(2), totals.margin.toFixed(2), "", ""],
    ];
    downloadCsv(`${fileStem(shopName)}-profitability-${from}_to_${to}.csv`, out);
  };
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
      <div>
        ...
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <h1 className="text-2xl font-black text-[#27324A] flex items-center gap-2">
            <TrendingUp className="h-6 w-6 text-[#A7653A]" /> Product profitability
          </h1>
          <Link href="/dashboard/owner/products/top" className="text-xs font-bold text-[#A7653A] hover:underline">
            Top products →
          </Link>
        </div>
        <p className="text-sm font-medium text-[#746E73] mt-1">
          Revenue, cost of goods sold (from batch consumption where available), and gross margin per product.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <Kpi label="Gross margin" value={money(totals.margin)} accent="#3da55e" />
      </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-[#f8f8f7] text-[10px] uppercase tracking-widest text-[#746E73] font-black">
                <tr>
                  ...
                  <SortTh label="Units" k="units" sortKey={sortKey} setSortKey={setSortKey} />
                  <SortTh label="Revenue" k="revenue" sortKey={sortKey} setSortKey={setSortKey} />
                  <th className="px-4 py-3 text-right">COGS</th>
                  <SortTh label="Margin" k="gross_margin" sortKey={sortKey} setSortKey={setSortKey} />
                  <SortTh label="Margin %" k="margin_pct" sortKey={sortKey} setSortKey={setSortKey} />
                </tr>
              </thead>
              <tbody className="divide-y divide-[#2E3344]/5">
                {sorted.map((r) => (
                  <tr key={r.product_id} className="hover:bg-[#f8f8f7]/50">
                    ...
                    <td className="px-4 py-3 text-right text-[#746E73]">{money(r.cogs)}</td>
                    <td className="px-4 py-3 text-right font-bold">{money(r.gross_margin)}</td>
                    <td className={`px-4 py-3 text-right font-bold ${r.margin_pct >= 0 ? "text-emerald-600" : "text-red-600"}`}>
                      {r.margin_pct}%
                    </td>
                  </tr>
                ))}
      ...
      <p className="text-[11px] text-[#746E73]">
        COGS is exact for products sold from batches; for non-batched products it&apos;s estimated from the current cost price.
      </p>
    </div>
  );
}
...
function SortTh({ label, k, sortKey, setSortKey }: { label: string; k: SortKey; sortKey: SortKey; setSortKey: (k: SortKey) => void }) {
  const active = sortKey === k;
  return (
    <th className="px-4 py-3 text-right">
      <button onClick={() => setSortKey(k)} className={`inline-flex items-center gap-1 ${active ? "text-[#27324A]" : ""}`}>
