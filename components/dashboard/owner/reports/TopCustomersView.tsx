...
export function TopCustomersView({ shopId, shopName }: Props) {
  ...
  const exportCsv = () => {
    ...
    downloadCsv(`${fileStem(shopName)}-top-customers.csv`, [
      ...
      ...rows.map((r, i) => [i + 1, r.name, r.phone ?? "", r.total_spent.toFixed(2), r.order_count, r.udhar_balance.toFixed(2)]),
    ]);
  };
    ...
    <div className="space-y-6 max-w-5xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
      <div className="flex items-end justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm font-medium text-[#746E73] mt-1">Your most valuable customers by lifetime spend or order count.</p>
        </div>
        <div className="flex gap-2 items-end">
          <div className="flex rounded-xl border border-[#2E3344]/15 overflow-hidden h-11">
            {([["spent", "By spend"], ["orders", "By orders"]] as const).map(([id, label]) => (
              <button key={id} onClick={() => setBy(id)}
                className={`px-3 text-xs font-bold ${by === id ? "bg-[#27324A] text-white" : "bg-white text-[#27324A]"}`}>{label}</button>
            ))}
          </div>
      ...
      <div className="bg-white rounded-2xl border border-[#2E3344]/8 shadow-sm overflow-hidden">
        {isPending ? (
          ...
          <div className="py-12 text-center text-sm font-bold text-[#746E73]">No customers recorded yet.</div>
        ) : (
                ...
                {rows.map((r, i) => (
                  <tr key={r.id} className="hover:bg-[#f8f8f7]/50">
                    ...
                    <td className="px-4 py-3">
                      ...
                      {r.phone && <p className="text-[11px] text-[#746E73]">{r.phone}</p>}
                    </td>
                    <td className="px-4 py-3 text-right font-bold">{money(r.total_spent)}</td>
                    <td className="px-4 py-3 text-right">{r.order_count}</td>
                    <td className={`px-4 py-3 text-right font-bold ${r.udhar_balance > 0 ? "text-[#A7653A]" : "text-[#746E73]"}`}>
