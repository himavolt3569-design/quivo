...
export function StockTakeView({ shopId, shopName, products, openTake, history }: Props) {
  const [search, setSearch] = useState("");
  const [counts, setCounts] = useState<Record<string, string>>(() => {
    const initial: Record<string, string> = {};
    for (const c of openTake?.counts ?? []) initial[c.product_id] = String(c.counted_qty);
    return initial;
  });
  ...
  const handleFinalise = () => {
    if (!openTake) return;
    if (!confirm("Finalise the stock take? Variances will become permanent stock adjustments.")) return;
    startTransition(async () => {
  ...
  const saveCount = async (productId: string, systemQty: number, raw: string) => {
    if (!openTake) return;
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <h1 className="text-2xl font-black text-[#27324A] flex items-center gap-2">
            <ClipboardCheck className="h-6 w-6 text-[#A7653A]" /> Stock take — {shopName}
          </h1>
      ...
      {openTake ? (
        <>
          <div className="bg-white rounded-2xl border border-[#2E3344]/8 shadow-sm p-4 flex items-center justify-between gap-3 flex-wrap">
            <div>
              <p className="text-xs font-black uppercase tracking-widest text-[#746E73]">Open stock take</p>
              <p className="text-sm font-bold text-[#27324A]">Started {fmt(openTake.row.started_at)}</p>
              <p className="text-[11px] text-[#A7653A] mt-1">
                {totalVariance} item{totalVariance === 1 ? "" : "s"} with non-zero variance
              </p>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-[#2E3344]/8 shadow-sm overflow-hidden">
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-[#2E3344]/5">
                  {filtered.map((p) => {
                    ...
                    const variance = counted === null ? null : counted - p.stock;
                    ...
                    return (
                      <tr key={p.id} className="hover:bg-[#f8f8f7]/50">
                        ...
                        <td className="px-4 py-3 text-right font-mono text-xs text-[#746E73]">{p.stock}</td>
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-end gap-2">
                            <input
                              ...
                              onBlur={(e) => void saveCount(p.id, p.stock, e.target.value)}
                              placeholder="—"
                              className="h-9 w-24 text-right px-2 border border-[#2E3344]/10 rounded-lg text-sm"
                            />
                            {savingId === p.id && <Loader2 className="h-3 w-3 animate-spin text-[#746E73]" />}
                          </div>
                        </td>
                        <td className={`px-4 py-3 text-right font-bold ${varianceClass}`}>
            ...
            <div className="p-4 border-t border-[#2E3344]/8 flex items-center justify-end gap-2">
              ...
              <span className="text-[11px] text-[#746E73]">Counts auto-save when you leave each field.</span>
            </div>
          </div>
        </>
      ) : (
        <div className="bg-white rounded-2xl border border-[#2E3344]/8 shadow-sm p-12 text-center">
          ...
          <p className="text-sm font-bold text-[#27324A]">No open stock take.</p>
          <p className="text-xs text-[#746E73] mt-1">Click <strong>Start stock take</strong> above to begin.</p>
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl border border-[#2E3344]/8 shadow-sm overflow-hidden">
        ...
        {history.length === 0 ? (
          <div className="py-8 text-center text-sm font-bold text-[#746E73]">No prior stock takes.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-[#f8f8f7] text-[10px] uppercase tracking-widest text-[#746E73] font-black">
              ...
              <tbody className="divide-y divide-[#2E3344]/5">
                {history.map((h) => (
                  <tr key={h.id}>
                    ...
                    <td className="px-4 py-3 text-xs text-[#746E73]">{h.notes ?? "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
