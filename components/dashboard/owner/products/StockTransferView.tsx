...
export function StockTransferView({ shops, activeShopId, sourceProducts, history }: Props) {
  const [fromShop, setFromShop] = useState(activeShopId);
  const [toShop, setToShop] = useState(shops.find((s) => s.id !== activeShopId)?.id ?? "");
  ...
  const filtered = useMemo(() => {
    ...
    return sourceProducts.filter((p) => p.name.toLowerCase().includes(q)).slice(0, 25);
  ...
  const submit = () => {
    if (!fromShop || !toShop) { toast.error("Pick source and destination shops."); return; }
    if (fromShop === toShop) { toast.error("Source and destination must differ."); return; }
    if (picked.length === 0) { toast.error("Add at least one line."); return; }
    const lines: { product_id: string; qty: number }[] = [];
    for (const l of picked) {
      const q = Number(l.qty);
      if (!Number.isFinite(q) || q <= 0) { toast.error("Each line needs qty > 0."); return; }
    ...
    <div className="space-y-6 max-w-6xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
      <div>
        ...
        <p className="text-sm font-medium text-[#746E73] mt-1">
          Move stock between your shops. The transfer is atomic — source decremented and destination batch created in a single transaction.
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-[#2E3344]/8 shadow-sm p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-[1fr_auto_1fr] gap-3 items-end">
          <div>
            <label className="text-[10px] font-black uppercase tracking-widest text-[#746E73]">From shop</label>
            <select
              value={fromShop}
            ...
            <p className="text-[10px] text-[#746E73] mt-1">
              Product list below is fetched from the source. Switch shops to change it.
            </p>
          </div>
          <ArrowRight className="h-5 w-5 text-[#A7653A] mx-auto hidden sm:block" />
          <div>
            <label className="text-[10px] font-black uppercase tracking-widest text-[#746E73]">To shop</label>
            <select
              value={toShop}
        ...
        {fromShop !== activeShopId && (
          <div className="p-3 rounded-xl bg-[#F7F0E6]/60 text-[11px] font-bold text-[#A7653A]">
            The product list below is from the currently-active shop. Switch the active shop to transfer from a different source.
          </div>
        )}

        {/* Product picker */}
        <div>
          <label className="text-[10px] font-black uppercase tracking-widest text-[#746E73]">Lines (from {shopNameById.get(activeShopId) ?? "active shop"})</label>
          <Input
            value={search}
            ...
            <div className="max-h-40 overflow-y-auto bg-[#f8f8f7] rounded-xl p-2 space-y-1 mt-2">
              {filtered.length === 0 ? (
                ...
                filtered.map((p) => (
                  ...
                  >
                    <span>{p.name}</span>
                    <span className="text-[11px] text-[#746E73] font-normal">stock {p.stock}{p.unit ? ` ${p.unit}` : ""}</span>
                  </button>
                ))
              )}
            </div>
        ...
        {picked.length === 0 ? (
          ...
          <ul className="divide-y divide-[#2E3344]/5 border border-[#2E3344]/10 rounded-xl overflow-hidden">
            {picked.map((l) => {
              ...
              return (
                <li key={l.id} className="grid grid-cols-[1fr_120px_36px] gap-2 items-center px-3 py-2 bg-white">
                  <div>
                    <p className="text-sm font-bold text-[#27324A]">{p?.name ?? "?"}</p>
                    <p className="text-[10px] text-[#746E73]">stock {p?.stock ?? 0}{p?.unit ? ` ${p.unit}` : ""}</p>
                  </div>
                  <Input
                    type="number"
                    inputMode="decimal"
                    ...
                    placeholder="qty"
                  />
                  <button onClick={() => removeLine(l.id)} className="h-9 w-9 rounded-lg border border-red-200 text-red-600 hover:bg-red-50 flex items-center justify-center">
      ...
      <div className="bg-white rounded-2xl border border-[#2E3344]/8 shadow-sm overflow-hidden">
        ...
        {history.length === 0 ? (
          <div className="py-8 text-center text-sm font-bold text-[#746E73]">No transfers yet.</div>
        ) : (
              ...
              <li key={t.id} className="p-4">
                <div className="flex items-center gap-2 text-xs font-bold text-[#27324A]">
                  ...
                  <span className={`ml-auto text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full ${
                    t.status === "completed" ? "bg-emerald-100 text-emerald-700" : t.status === "cancelled" ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-700"
                  }`}>{t.status}</span>
                </div>
                <p className="text-[11px] text-[#746E73] mt-1">{fmt(t.created_at)}</p>
                {t.lines.length > 0 && (
                  <ul className="text-[11px] text-[#27324A] mt-2 space-y-0.5">
                    {t.lines.map((l) => {
                      const product = Array.isArray(l.product) ? l.product[0] : l.product;
                      return (
                        <li key={l.id}>· {product?.name ?? "?"} × {l.qty}</li>
                      );
                    })}
                  </ul>
                )}
                {t.notes && <p className="text-[11px] text-[#746E73] italic mt-2">&quot;{t.notes}&quot;</p>}
              </li>
            ))}
          </ul>
        )}
