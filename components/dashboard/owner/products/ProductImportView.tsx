...
import {
  ChevronLeft, Upload, FileText, Download, CheckCircle2, AlertCircle, Loader2,
} from "lucide-react";
import { toast } from "sonner";
import { bulkImportProducts, type ImportReport } from "@/app/actions/product-import";
...
const AUTO_MAP: Record<string, keyof typeof FIELD_LABELS> = {
  ...
  price: "price", sell: "price", "selling price": "price", mrp: "price",
  cost: "cost_price", "cost price": "cost_price", buy: "cost_price",
  stock: "stock", qty: "stock", quantity: "stock", "on hand": "stock",
  threshold: "low_stock_threshold", "low stock": "low_stock_threshold", "low stock threshold": "low_stock_threshold", reorder: "low_stock_threshold",
  barcode: "barcode", ean: "barcode", upc: "barcode", sku: "barcode",
};
...
function autoMapHeader(header: string): keyof typeof FIELD_LABELS | "" {
  ...
  if (FIELD_IDS.includes(key as keyof typeof FIELD_LABELS)) return key as keyof typeof FIELD_LABELS;
  return "";
}

function downloadTemplate() {
  const header = ["name", "brand", "category", "unit", "variant", "description", "price", "cost_price", "stock", "low_stock_threshold", "barcode"];
  const sample = [
    "Wai Wai Noodles", "Wai Wai", "Snacks", "30 g", "Chicken", "Instant noodle pack", "20", "16", "50", "10", "8901764012345",
  ];
...
function validateRow(mapped: Record<string, string>): string | null {
  ...
  if (mapped.cost_price && Number.isNaN(Number(mapped.cost_price))) return "Cost price is not a number";
  if (mapped.stock && Number.isNaN(Number(mapped.stock))) return "Stock is not a number";
  if (mapped.low_stock_threshold && Number.isNaN(Number(mapped.low_stock_threshold))) return "Threshold is not a number";
  return null;
}
...
export function ProductImportView({ shopId }: Props) {
  ...
  const [mapping, setMapping] = useState<Record<number, keyof typeof FIELD_LABELS | "">>({});
  ...
  const ingestCsv = (text: string) => {
    ...
    headerRow.forEach((h, i) => { initialMap[i] = autoMapHeader(h); });
  ...
  const canSubmit = parsedRows.length > 0 && nameAssigned && priceAssigned && goodCount > 0 && !isPending;
  ...
  const submit = () => {
    const goodRows = parsedRows
      ...
      .map((r) => ({
        ...
        low_stock_threshold: r.mapped.low_stock_threshold ? Number(r.mapped.low_stock_threshold) : null,
    ...
    startTransition(async () => {
      ...
      if (res.report) {
        ...
        toast.success(`Imported ${res.report.inserted} new, updated ${res.report.updated}.`);
      }
    });
  };
    ...
    <div className="space-y-6 max-w-6xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
      <div>
        ...
        <p className="text-sm font-medium text-[#746E73] mt-1">
          Paste or upload a CSV. We auto-detect columns; you can review the mapping before committing.
          Rows with a matching barcode update in place (toggle below to switch).
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-[#2E3344]/8 shadow-sm p-6 space-y-4">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <h2 className="text-sm font-black uppercase tracking-widest text-[#746E73]">1. Source</h2>
        ...
        <label className="flex items-center gap-2 text-xs font-bold text-[#27324A]">
          ...
          />
          Update existing products when barcode matches (otherwise duplicates are created)
        </label>
      </div>
      ...
      {headers.length > 0 && (
        <div className="bg-white rounded-2xl border border-[#2E3344]/8 shadow-sm p-6 space-y-4">
          <h2 className="text-sm font-black uppercase tracking-widest text-[#746E73]">2. Column mapping</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {headers.map((h, i) => (
              <div key={i}>
                <p className="text-[10px] font-black uppercase tracking-widest text-[#746E73] truncate">{h || `(column ${i + 1})`}</p>
                <select
                  value={mapping[i] ?? ""}
                  onChange={(e) =>
                    setMapping((prev) => ({ ...prev, [i]: e.target.value as keyof typeof FIELD_LABELS | "" }))
                  }
                  className="h-10 px-2 rounded-lg border border-[#2E3344]/15 bg-white text-sm font-bold w-full mt-1"
                >
                  <option value="">— ignore —</option>
                  {FIELD_IDS.map((id) => (
                    <option key={id} value={id}>{FIELD_LABELS[id]}</option>
                  ))}
                </select>
              </div>
            ))}
          ...
          {(!nameAssigned || !priceAssigned) && (
            <div className="p-3 rounded-xl bg-red-50 border border-red-200 text-[11px] font-bold text-red-700 flex items-start gap-2">
              ...
              <span>Both <em>Name</em> and <em>Price</em> mappings are required.</span>
            </div>
          )}
        </div>
      )}
      ...
      {parsedRows.length > 0 && (
        <div className="bg-white rounded-2xl border border-[#2E3344]/8 shadow-sm overflow-hidden">
          <div className="p-4 border-b border-[#2E3344]/8 flex items-center gap-3 flex-wrap">
            <h2 className="text-sm font-black uppercase tracking-widest text-[#746E73]">3. Preview</h2>
            <span className="text-[11px] font-bold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-full">{goodCount} OK</span>
            {errorCount > 0 && (
              <span className="text-[11px] font-bold text-red-700 bg-red-50 px-2 py-0.5 rounded-full">{errorCount} with issues</span>
            )}
          </div>
          <div className="overflow-x-auto max-h-[420px]">
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y divide-[#2E3344]/5">
                {parsedRows.slice(0, 500).map((r, idx) => (
                  <tr key={idx} className={r.error ? "bg-red-50/40" : "hover:bg-[#f8f8f7]/50"}>
                    <td className="px-3 py-2 text-[#746E73]">{idx + 1}</td>
                    <td className="px-3 py-2">
                      {r.error
                        ? <span className="text-red-700 inline-flex items-center gap-1"><AlertCircle className="h-3 w-3" /> {r.error}</span>
                        : <span className="text-emerald-700 inline-flex items-center gap-1"><CheckCircle2 className="h-3 w-3" /> OK</span>}
                    </td>
                    <td className="px-3 py-2 font-bold text-[#27324A]">{r.mapped.name ?? "—"}</td>
                    <td className="px-3 py-2 text-right">{r.mapped.price ?? "—"}</td>
                    <td className="px-3 py-2 text-right">{r.mapped.stock ?? "0"}</td>
                    <td className="px-3 py-2 font-mono">{r.mapped.barcode ?? "—"}</td>
            ...
            {parsedRows.length > 500 && (
              <div className="p-3 text-[11px] font-bold text-[#746E73] text-center">
                Showing first 500 rows. All {parsedRows.length} rows will be submitted.
              </div>
            )}
          </div>
        </div>
      ...
      {parsedRows.length > 0 && (
        <div className="bg-white rounded-2xl border border-[#2E3344]/8 shadow-sm p-4 flex flex-wrap items-center justify-between gap-3">
          <p className="text-xs text-[#746E73] font-bold">
            Ready to import <span className="text-[#27324A]">{goodCount}</span> row{goodCount === 1 ? "" : "s"}
      ...
      {report && (
        <div className="bg-white rounded-2xl border border-emerald-200 shadow-sm p-6 space-y-2">
          ...
          <div className="grid grid-cols-3 gap-3 text-center">
            <div>
              <p className="text-3xl font-black text-emerald-700">{report.inserted}</p>
              <p className="text-[10px] uppercase tracking-widest font-bold text-[#746E73] mt-1">Inserted</p>
            </div>
            <div>
              <p className="text-3xl font-black text-[#27324A]">{report.updated}</p>
              <p className="text-[10px] uppercase tracking-widest font-bold text-[#746E73] mt-1">Updated</p>
            </div>
            <div>
              <p className="text-3xl font-black text-red-600">{report.skipped}</p>
              <p className="text-[10px] uppercase tracking-widest font-bold text-[#746E73] mt-1">Skipped</p>
            </div>
          </div>
          {report.errors.length > 0 && (
            <details className="mt-3 text-xs font-bold text-red-600">
              <summary className="cursor-pointer">{report.errors.length} error{report.errors.length === 1 ? "" : "s"}</summary>
              <ul className="mt-2 space-y-1">
                {report.errors.slice(0, 20).map((e, i) => (
                  <li key={i}>row {e.index}: {e.message}</li>
                ))}
              </ul>
            </details>
          )}
