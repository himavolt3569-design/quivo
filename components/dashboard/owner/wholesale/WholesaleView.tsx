...
export function WholesaleView({
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div>
        <h1 className="text-2xl font-black text-[#0F172A] tracking-tight">Wholesale B2B</h1>
        <p className="text-[#746E73] text-sm mt-1">
          Manage retailer applications and custom pricing. Your global wholesale discount is <strong className="text-[#0F172A]">{globalDiscount}%</strong>.
        </p>
      </div>
      ...
      <div className="space-y-6">
        <section>
          ...
          {pending.length === 0 ? (
            <div className="bg-white border border-[#1E293B]/8 rounded-[2rem] p-8 text-center">
              <p className="text-[#746E73]">No pending applications at the moment.</p>
            </div>
          ) : (
            <div className="grid gap-4 md:grid-cols-2">
              {pending.map((app) => (
                <div key={app.id} className="bg-white border border-[#1E293B]/8 rounded-[2rem] p-6 shadow-sm flex flex-col gap-4">
                  <div className="flex items-start gap-4">
                    <div className="h-12 w-12 rounded-full bg-[#F8FAFC] flex items-center justify-center shrink-0">
                      <Store className="h-6 w-6 text-[#3B82F6]" />
                    </div>
                    <div>
                      <h3 className="font-bold text-[#0F172A] text-lg">{app.retailer.name}</h3>
                  ...
                  <div className="flex items-center gap-2 mt-auto pt-4 border-t border-[#1E293B]/8">
                    <button
                      disabled={loadingId === app.id}
                      onClick={() => handleReject(app.id)}
                      className="flex-1 flex justify-center items-center gap-2 py-2.5 rounded-xl border border-[#1E293B]/10 text-[#746E73] font-semibold hover:bg-gray-50 transition-colors disabled:opacity-50"
                    >
                    ...
                    <button
                      ...
                      className="flex-1 flex justify-center items-center gap-2 py-2.5 rounded-xl bg-[#0F172A] text-white font-semibold hover:bg-[#1f293b] transition-colors shadow-md disabled:opacity-50"
                    >
        ...
        <section>
          <h2 className="text-lg font-bold text-[#0F172A] flex items-center gap-2 mb-4 mt-10">
          ...
          {approved.length === 0 ? (
            ...
            <div className="bg-white border border-[#1E293B]/8 rounded-[2rem] overflow-hidden shadow-sm">
              <table className="w-full text-sm text-left">
                <thead className="bg-[#F8F8F7] text-[#746E73] uppercase text-[10px] font-bold tracking-wider border-b border-[#1E293B]/8">
                  <tr>
                ...
                <tbody className="divide-y divide-[#1E293B]/8">
                  {approved.map((app) => (
                    <tr key={app.id} className="hover:bg-gray-50/50 transition-colors">
                      <td className="px-6 py-4 font-bold text-[#0F172A]">{app.retailer.name}</td>
                        ...
                        <div className="flex items-center gap-2 max-w-[120px]">
                          <div className="relative flex-1">
                            <input
                              ...
                              className="w-full pl-3 pr-8 py-2 bg-white border border-[#1E293B]/20 rounded-xl text-[#0F172A] focus:outline-none focus:ring-2 focus:ring-[#3B82F6]/20"
                            />
                            <Percent className="absolute right-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#746E73]" />
                          </div>
                        </div>
                        {app.override_discount_percent !== null && (
                          <span className="text-[10px] text-[#3B82F6] font-medium mt-1 block">Custom applied</span>
                        )}
                      </td>
