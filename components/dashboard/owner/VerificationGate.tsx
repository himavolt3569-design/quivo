...
const EXEMPT_PATHS = ["/dashboard/owner/settings", "/onboarding/owner"];
...
export function VerificationGate({
  ...
  const configs: Partial<
    Record<
      VerificationStatus,
      { title: string; sub: string; cta: string | null }
    >
  > = {
    unverified: {
  ...
  const cfg = configs[status] ?? {
    ...
    sub: "Your shop needs business proof.",
    cta: "Verify Now",
  };
  ...
  return (
    <>
      ...
      <div
        className="pointer-events-none select-none opacity-20 blur-[2px]"
        aria-hidden="true"
      >
        {children}
      </div>
