...
import {
  Banknote, Calendar, Download, RefreshCw, Plus, Trash2, History, X, ChevronDown,
} from "lucide-react";
...
import {
  getPayrollSummary, getPayrollLines, setStaffRate, deleteStaffRate,
  type PayrollSummaryRow, type PayrollLine,
...
export function PayrollView({ shopId, shopName, staff, ratesByStaff }: Props) {
  ...
  const [start, setStart] = useState<string>(() => toDateInput(startOfWeek(today)));
  ...
  const [rates, setRates] = useState<Record<string, StaffRateRow[]>>(ratesByStaff);
  ...
  const loadSummary = useMemo(() => {
    return async () => {
      ...
      const res = await getPayrollSummary({ shopId, start: startIso, end: endIso });
      setLoading(false);
  ...
  const handleExport = async () => {
    ...
    if (lines.length === 0) { toast.info("No completed shifts in range — nothing to export."); return; }
    const header = [
      "Shift ID", "Staff", "Scheduled start", "Scheduled end",
      "Clock in", "Clock out", "Hours worked", "Rate", "Pay", "Currency",
    ];
    const rows = [header, ...lines.map((l) => [
      l.shift_id, l.staff_name,
      l.scheduled_start, l.scheduled_end,
      l.clocked_in_at, l.clocked_out_at,
      l.worked_hours.toFixed(2), Number(l.rate).toFixed(2),
      Number(l.pay).toFixed(2), l.currency,
    ])];
    const fname = `${shopName.replace(/[^a-z0-9]+/gi, "-").toLowerCase()}-payroll-${start}-to-${end}.csv`;
    downloadCsv(fname, rows);
    toast.success(`Exported ${lines.length} shift line${lines.length === 1 ? "" : "s"}`);
  };

  const handleSetRate = (e: React.FormEvent) => {
    e.preventDefault();
    const value = Number(rateForm.hourlyRate);
    if (!rateForm.staffId) { toast.error("Pick a staff member"); return; }
    if (!Number.isFinite(value) || value < 0) { toast.error("Enter a valid rate"); return; }
    ...
    startTransition(async () => {
      ...
      setRates((prev) => {
        const list = (prev[rateForm.staffId] ?? []).filter((r) => r.id !== newRow.id);
        return { ...prev, [rateForm.staffId]: [newRow, ...list].sort((a, b) => b.effective_from.localeCompare(a.effective_from)) };
      });
  ...
  const handleDeleteRate = (rateId: string, staffId: string) => {
    if (!confirm("Delete this rate? Past payroll using it stays unchanged.")) return;
    startTransition(async () => {
    ...
    <div className="max-w-6xl mx-auto space-y-6 pb-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-4">
        <div>
          ...
          <p className="text-sm font-medium text-[#746E73] mt-1">
            Hours worked × hourly rate. Past pay stays locked to the rate that was effective at shift time.
          </p>
        </div>
        <Button
        ...
        >
          <Download className="h-4 w-4 mr-2" /> {exporting ? "Preparing…" : "Export CSV"}
        </Button>
      </div>
      ...
      <section className="bg-white rounded-[2rem] border border-[#2E3344]/8 shadow-sm p-5 sm:p-6">
        <div className="flex flex-wrap items-end gap-3">
          <div>
            <Label className="text-[10px] font-black uppercase tracking-wider text-[#746E73]">From</Label>
            <Input type="date" value={start} onChange={(e) => setStart(e.target.value)} max={end} className="mt-1 h-10 rounded-xl w-44" />
          </div>
          <div>
            <Label className="text-[10px] font-black uppercase tracking-wider text-[#746E73]">To</Label>
            <Input type="date" value={end} onChange={(e) => setEnd(e.target.value)} min={start} className="mt-1 h-10 rounded-xl w-44" />
          </div>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white rounded-[1.5rem] border border-[#2E3344]/8 p-5">
          <p className="text-[10px] font-black uppercase tracking-widest text-[#746E73]">Shifts</p>
          <p className="text-2xl font-black text-[#27324A] mt-1">{totals.totalShifts}</p>
        </div>
        <div className="bg-white rounded-[1.5rem] border border-[#2E3344]/8 p-5">
          <p className="text-[10px] font-black uppercase tracking-widest text-[#746E73]">Hours worked</p>
          <p className="text-2xl font-black text-[#27324A] mt-1">{totals.totalHours.toFixed(2)}</p>
        </div>
        <div className="bg-white rounded-[1.5rem] border border-[#A7653A]/30 p-5 bg-[#F7F0E6]/40">
          <p className="text-[10px] font-black uppercase tracking-widest text-[#A7653A]">Total payable</p>
          <p className="text-2xl font-black text-[#A7653A] mt-1">{fmtMoney(totals.totalPay, totals.currency)}</p>
        </div>
      </section>
      ...
      <section className="bg-white rounded-[2rem] border border-[#2E3344]/8 shadow-sm overflow-hidden">
        <div className="p-5 border-b border-[#2E3344]/8 flex items-center justify-between">
          ...
          {loading && <span className="text-xs text-[#746E73] font-medium animate-pulse">Loading…</span>}
        </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-[#f8f8f7] text-[10px] uppercase tracking-widest text-[#746E73]">
                <tr>
                  ...
                  <th className="text-right font-black px-5 py-3">Scheduled hrs</th>
                  <th className="text-right font-black px-5 py-3">Worked hrs</th>
                  <th className="text-right font-black px-5 py-3">Current rate</th>
                  ...
                  return (
                    <Fragment key={row.staff_id}>
                      <tr className="border-t border-[#2E3344]/5">
                        <td className="px-5 py-3 font-bold text-[#27324A]">{row.staff_name}</td>
                        <td className="px-5 py-3 text-right tabular-nums">{row.shifts_count}</td>
                        <td className="px-5 py-3 text-right tabular-nums text-[#746E73]">{Number(row.scheduled_hours).toFixed(2)}</td>
                        <td className="px-5 py-3 text-right tabular-nums font-bold">{Number(row.worked_hours).toFixed(2)}</td>
                        <td className="px-5 py-3 text-right tabular-nums">
                          {row.current_rate !== null && row.current_rate !== undefined
                            ? fmtMoney(Number(row.current_rate), row.currency)
                            : <span className="text-red-500 text-xs font-bold">Not set</span>}
                        </td>
                        <td className="px-5 py-3 text-right tabular-nums font-black text-[#A7653A]">{fmtMoney(Number(row.total_pay), row.currency)}</td>
                        <td className="px-2">
                          <button
                            onClick={() => setExpandedStaff(open ? null : row.staff_id)}
                      ...
                      {open && (
                        <tr className="bg-[#F7F0E6]/30">
                          <td colSpan={7} className="px-5 py-4">
                            ...
                            {history.length === 0 ? (
                              <p className="text-xs text-[#746E73]">No rates set. Add one below.</p>
                            ) : (
                              <ul className="space-y-1.5">
                                {history.map((r) => (
                                  <li key={r.id} className="flex items-center justify-between text-xs bg-white border border-[#2E3344]/5 rounded-lg px-3 py-2">
                                    <div>
                                      <span className="font-black text-[#27324A]">{fmtMoney(Number(r.hourly_rate), r.currency)}</span>
                                      <span className="text-[#746E73] ml-3">from {fmtDateTime(r.effective_from)}</span>
                                      {r.note && <span className="text-[#746E73] italic ml-2">— {r.note}</span>}
                                    </div>
                                    <button
                                      onClick={() => handleDeleteRate(r.id, row.staff_id)}
      ...
      <section className="bg-white rounded-[2rem] border border-[#2E3344]/8 shadow-sm p-5 sm:p-6">
        ...
        <p className="text-xs text-[#746E73] mb-4">
          New rates apply from the chosen date forward. Past shifts keep the rate that was active at the time.
        </p>
        <form onSubmit={handleSetRate} className="grid sm:grid-cols-5 gap-3">
          <div className="sm:col-span-2">
            <Label className="text-[10px] font-black uppercase tracking-wider text-[#746E73]">Staff</Label>
            <Select value={rateForm.staffId} onValueChange={(v) => setRateForm((f) => ({ ...f, staffId: v }))}>
          ...
          <div>
            <Label className="text-[10px] font-black uppercase tracking-wider text-[#746E73]">Rate / hour</Label>
            <Input
          ...
          <div>
            <Label className="text-[10px] font-black uppercase tracking-wider text-[#746E73]">Currency</Label>
            <Input
              value={rateForm.currency}
              onChange={(e) => setRateForm((f) => ({ ...f, currency: e.target.value.toUpperCase().slice(0, 3) }))}
              maxLength={3}
              className="mt-1 h-11 rounded-xl uppercase"
            />
          </div>
          <div>
            <Label className="text-[10px] font-black uppercase tracking-wider text-[#746E73]">Effective from</Label>
            <Input
              type="date"
          ...
          <div className="sm:col-span-4">
            <Label className="text-[10px] font-black uppercase tracking-wider text-[#746E73]">Note (optional)</Label>
            <Input
              value={rateForm.note}
