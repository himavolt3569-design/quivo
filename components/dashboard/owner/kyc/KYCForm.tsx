...
export function KYCForm({
  ...
  const handleSubmit = async () => {
    if (!files.length && !kycDocumentUrls.length) {
    ...
    for (const file of files) {
      ...
      const {
        data: { publicUrl },
      } = supabase.storage.from("shop_assets").getPublicUrl(up.path);
  ...
  return (
    <div className="space-y-6">
      {/* Status Card */}
      <div
        className={`rounded-4xl border p-6 ${cfg.bgClass} border-current/10`}
      >
        <div className="flex items-start gap-4">
          <div
            className={`h-12 w-12 rounded-2xl flex items-center justify-center shrink-0 bg-white shadow-sm`}
          >
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-1">
              ...
              <span
                className={`text-xs font-black px-2.5 py-1 rounded-full uppercase tracking-wider ${cfg.badgeClass}`}
              >
                {cfg.label}
              </span>
            </div>
            ...
            {kycSubmittedAt && verificationStatus === "pending" && (
              <p className="text-xs text-[#746E73] mt-2 font-medium">
                Submitted{" "}
                {new Date(kycSubmittedAt).toLocaleDateString("en-IN", {
                  day: "numeric",
                  month: "long",
                  year: "numeric",
                })}
      ...
      {verificationStatus === "rejected" && kycRejectionReason && (
        <div className="rounded-3xl bg-red-50 border border-red-200 p-5">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="font-black text-sm text-red-800 mb-1">
                Rejection Reason
              </p>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-[#746E73]">
                This page updates automatically. Typical review time: 24–48
                hours.
              </p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-4xl border border-[#2E3344]/10 p-6 space-y-5">
          <div>
            ...
            <p className="text-sm text-[#746E73]">
              Accepted: PAN card, citizenship, trade license, or business
              registration. Max 5 MB each.
            </p>
          </div>
          ...
          <label className="flex flex-col items-center justify-center gap-3 p-8 border-2 border-dashed border-[#2E3344]/15 rounded-2xl cursor-pointer hover:border-[#A7653A]/50 hover:bg-[#F7F0E6]/30 transition group">
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-[#746E73]">
                JPG, PNG, WEBP, PDF — up to 5 files, 5 MB each
              </p>
            </div>
            <input
              type="file"
      ...
      {verificationStatus === "verified" && (
        <div className="flex flex-col items-center py-8 gap-3 text-center">
          ...
          <p className="text-sm text-[#746E73]">
            Your shop is verified and all features are unlocked.
          </p>
        </div>
      )}
    </div>
