...
import {
  Plus, Trash2, PackagePlus, ClipboardList, ChevronDown, ChevronUp, X, Loader2,
} from "lucide-react";
...
export function PurchaseOrdersView({ shopId, supplierId, supplierName, products, initialOrders }: Props) {
  ...
  // Create-PO state
  const [draftLines, setDraftLines] = useState<Array<{
    product_id: string;
    qty: string;
    unit_cost: string;
  ...
  const [receiveQty, setReceiveQty] = useState<Record<string, Record<string, string>>>({});
  ...
  const draftTotal = useMemo(
    () => draftLines.reduce((a, l) => a + (Number(l.qty) || 0) * (Number(l.unit_cost) || 0), 0),
    [draftLines]
  );
  ...
  const updateLine = (productId: string, key: "qty" | "unit_cost" | "expected_expiry", value: string) => {
    setDraftLines((prev) => prev.map((l) => (l.product_id === productId ? { ...l, [key]: value } : l)));
  };
  ...
  const submitCreate = () => {
    if (draftLines.length === 0) { toast.error("Add at least one product line."); return; }
  ...
  const submitReceive = (poId: string) => {
    ...
    const lines: { line_id: string; qty: number; batch_no?: string | null; expiry_date?: string | null }[] = [];
    for (const [line_id, raw] of Object.entries(lineMap)) {
      const qty = Number(raw);
      if (!Number.isFinite(qty) || qty <= 0) continue;
      lines.push({ line_id, qty });
    }
    if (lines.length === 0) { toast.error("Enter at least one received qty."); return; }
    startTransition(async () => {
      ...
      setOrders((prev) => prev.map((po) => {
        ...
        const allDone = next.lines.every((l) => l.qty_received >= l.qty_ordered);
  ...
  const handleCancel = (poId: string) => {
    ...
    startTransition(async () => {
      ...
      if (res.error) { toast.error(res.error); return; }
      setOrders((prev) => prev.map((po) => (po.id === poId ? { ...po, status: "cancelled" } : po)));
      ...
      {createOpen && (
        <div className="bg-white rounded-2xl border border-[#27324A]/15 shadow-sm p-4 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="text-[10px] font-black uppercase tracking-widest text-[#746E73]">Expected delivery</label>
              <Input type="date" value={expectedAt} onChange={(e) => setExpectedAt(e.target.value)} className="h-11 rounded-xl mt-1" />
            </div>
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-widest text-[#746E73]">Lines</label>
            <Input
              value={productPicker}
            ...
            {productPicker.trim() && (
              <div className="max-h-40 overflow-y-auto bg-[#f8f8f7] rounded-xl p-2 space-y-1">
                {filteredProducts.length === 0 ? (
                  <p className="text-xs text-[#746E73] text-center py-2">No match.</p>
                ) : (
                  filteredProducts.map((p) => (
                    ...
                    >
                      <span>{p.name}</span>
                      <span className="text-[11px] text-[#746E73] font-normal">stock {p.stock}</span>
                    </button>
                  ))
                )}
              </div>
            ...
            {draftLines.length === 0 ? (
              <div className="text-center py-6 text-xs text-[#746E73] font-bold">No lines yet.</div>
            ) : (
              <ul className="divide-y divide-[#2E3344]/5 border border-[#2E3344]/10 rounded-xl overflow-hidden">
                {draftLines.map((l) => {
                  const p = productById.get(l.product_id);
                  return (
                    <li key={l.product_id} className="grid grid-cols-[1fr_80px_110px_120px_36px] gap-2 items-center px-3 py-2 bg-white">
                      <div>
                        <p className="text-sm font-bold text-[#27324A]">{p?.name ?? "?"}</p>
                        <p className="text-[10px] text-[#746E73]">{p?.unit ?? ""}</p>
                      </div>
                      <Input
                        type="number" inputMode="decimal" step="0.001" min="0"
                        value={l.qty} onChange={(e) => updateLine(l.product_id, "qty", e.target.value)}
                        className="h-9 text-right text-sm rounded-lg" placeholder="qty"
                      />
                      <Input
                        type="number" inputMode="decimal" step="0.01" min="0"
                        value={l.unit_cost} onChange={(e) => updateLine(l.product_id, "unit_cost", e.target.value)}
                        className="h-9 text-right text-sm rounded-lg" placeholder="cost"
                      />
                      <Input
                        type="date" value={l.expected_expiry}
                        onChange={(e) => updateLine(l.product_id, "expected_expiry", e.target.value)}
                        className="h-9 text-sm rounded-lg"
                      />
                      <button onClick={() => removeLine(l.product_id)} className="h-9 w-9 rounded-lg border border-red-200 text-red-600 hover:bg-red-50 flex items-center justify-center">
          ...
          <div className="flex items-center justify-between border-t border-[#2E3344]/8 pt-4">
            <p className="text-sm font-bold text-[#27324A]">Total: <span className="text-lg font-black text-[#A7653A]">{money(draftTotal)}</span></p>
            <div className="flex gap-2">
              <button onClick={resetCreate} className="h-10 px-3 rounded-xl border border-[#2E3344]/10 font-bold text-xs">Cancel</button>
              <button
      ...
      {orders.length === 0 ? (
        ...
        <ul className="space-y-2">
          {orders.map((po) => {
            ...
            const receivable = po.status === "submitted" || po.status === "partial";
            return (
              <li key={po.id} className="bg-white rounded-2xl border border-[#2E3344]/8 shadow-sm">
                <button
                  type="button"
                  onClick={() => setOpen((prev) => ({ ...prev, [po.id]: !prev[po.id] }))}
                  className="w-full text-left p-4 flex items-center gap-3 hover:bg-[#f8f8f7]/50 transition"
                >
                  <span className={`px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider ${STATUS_BADGE[po.status]}`}>
                    {po.status}
                  </span>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-[#27324A]">
                      {money(po.total_amount)} · {po.lines.length} line{po.lines.length === 1 ? "" : "s"}
                    </p>
                    <p className="text-[11px] text-[#746E73]">
                      ordered {fmt(po.ordered_at)}{po.expected_at ? ` · expected ${po.expected_at}` : ""}
                    </p>
                  </div>
                  {isOpen ? <ChevronUp className="h-4 w-4 text-[#746E73]" /> : <ChevronDown className="h-4 w-4 text-[#746E73]" />}
                </button>
                {isOpen && (
                  <div className="border-t border-[#2E3344]/8 p-4 space-y-3">
                    <table className="w-full text-xs">
                      <thead className="text-[10px] uppercase tracking-widest text-[#746E73] font-black">
                        <tr>
                          ...
                          {receivable && <th className="text-right w-28">Receive now</th>}
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-[#2E3344]/5">
                        {po.lines.map((l) => {
                          ...
                          return (
                            <tr key={l.id}>
                              <td className="py-2 font-bold text-[#27324A]">{l.product_name}</td>
                              ...
                              {receivable && (
                                <td className="text-right">
                                  <Input
                                    type="number" inputMode="decimal" step="0.001" min="0" max={remaining}
                                    value={receiveQty[po.id]?.[l.id] ?? ""}
                                    onChange={(e) =>
                                      setReceiveQty((prev) => ({
                                        ...prev,
                                        [po.id]: { ...(prev[po.id] ?? {}), [l.id]: e.target.value },
                                      }))
                                    }
                                    placeholder="0"
                    ...
                    {po.notes && <p className="text-[11px] text-[#746E73] italic">&quot;{po.notes}&quot;</p>}
                    <div className="flex justify-end gap-2 pt-2 border-t border-[#2E3344]/5">
                      ...
                      {(po.status === "submitted" || po.status === "draft" || po.status === "partial") && (
                        <button
