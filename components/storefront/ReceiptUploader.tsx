    ...
    return (
      <div className="bg-green-50 border border-green-200 rounded-3xl p-5 flex items-center gap-3">
        ...
        <p className="text-sm font-bold text-green-900">
          Receipt uploaded — the shop will verify shortly.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white border border-blue-200 rounded-3xl shadow-sm p-5 space-y-4">
      ...
      <p className="text-xs text-gray-600">
        ...
        using the details below, then upload a screenshot/photo or PDF of the
        receipt.
      </p>
      ...
      {method === "bank_transfer" && bank && (
        <div className="space-y-1.5">
          ...
          {bank.bank_account_holder && (
            <Copyable label="Holder" value={bank.bank_account_holder} />
          )}
          {bank.bank_account_number && (
            <Copyable label="Account #" value={bank.bank_account_number} />
          )}
          {bank.bank_branch && (
            <Copyable label="Branch" value={bank.bank_branch} />
          )}
          {bank.bank_swift_code && (
            <Copyable label="SWIFT" value={bank.bank_swift_code} />
          )}
          {bank.payment_instructions && (
            <p className="text-[11px] text-gray-600 whitespace-pre-wrap pt-1">
              {bank.payment_instructions}
            </p>
          )}
        </div>
      ...
      {method === "qr_code" && bank?.qr_code_url && (
        <div className="flex flex-col items-center gap-2">
          ...
          <img
            ...
            alt="Payment QR"
            className="h-52 w-52 rounded-xl border border-gray-200 bg-white object-contain"
          />
          {bank.payment_instructions && (
            <p className="text-[11px] text-gray-600 whitespace-pre-wrap text-center">
              {bank.payment_instructions}
            </p>
          )}
        </div>
      ...
      {preview && (
        <img
          src={preview}
          alt="Preview"
          className="h-32 w-full object-contain rounded-xl border border-gray-100"
        />
      )}
