...
export function WholesaleBanner({ wholesalerShopId, retailerShopId, status }: Props) {
  ...
  if (currentStatus === "approved") {
    return (
      <div className="fixed bottom-0 left-0 right-0 z-40 bg-[#F8FAFC] border-t border-[#3B82F6]/20 p-3 shadow-[0_-4px_10px_rgba(0,0,0,0.05)]">
  ...
  if (currentStatus === "pending") {
    return (
      <div className="fixed bottom-0 left-0 right-0 z-40 bg-[#f8f8f7] border-t border-[#1E293B]/10 p-3 shadow-[0_-4px_10px_rgba(0,0,0,0.05)]">
  ...
  // Not applied yet
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-[#1E293B]/10 p-4 pb-safe shadow-[0_-10px_20px_rgba(0,0,0,0.05)]">
      <div className="max-w-screen-md mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
        <div>
          <h3 className="font-bold text-[#0F172A] text-sm flex items-center gap-2">
            <Store className="h-4 w-4 text-[#3B82F6]" /> Wholesale Partner Program
          </h3>
        ...
        <button
          onClick={handleApply}
          disabled={loading}
          className="w-full sm:w-auto shrink-0 bg-[#0F172A] hover:bg-[#1f293b] text-white px-6 py-2.5 rounded-xl font-bold text-sm shadow-md transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2"
        >
