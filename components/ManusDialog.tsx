...
export function ManusDialog({
  ...
  useEffect(() => {
    ...
    if (isVerificationPending && timeLeft > 0) {
      timer = setTimeout(() => setTimeLeft((prev) => prev - 1), 1000);
    }
    return () => clearTimeout(timer);
  }, [isVerificationPending, timeLeft]);
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // Every Google button in this dialog represents owner intent (the login
      // CTA, and the owner-tab signup CTA). Carry that through the OAuth
      // round-trip so /auth/callback can route to /onboarding/owner for new
      // owners and /dashboard/owner for returning ones.
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${window.location.origin}/auth/callback?intent=owner`,
          queryParams: {
            access_type: 'offline',
            prompt: 'consent',
          }
    ...
    >
      <DialogContent className="w-[92vw] sm:w-[400px] max-w-[400px] max-h-[90vh] overflow-y-auto bg-[#f8f8f7] rounded-[24px] shadow-2xl border border-[rgba(0,0,0,0.08)] backdrop-blur-2xl p-0 gap-0">
        <DialogHeader className="flex flex-col items-center gap-2 pt-6 pb-2 px-4 sm:px-6">
          ...
          <DialogDescription className="text-sm text-[#858481] text-center px-2 sm:px-4">
            {isVerificationPending
              ? "We've sent a verification link to your email address. Please click the link to verify your account."
              : isForgotPassword
                ? "Enter your email and we will send you a reset link."
                : (isLogin ? "Enter your credentials to access your account" : "Select your role and sign up to continue")}
          </DialogDescription>
        </DialogHeader>
        ...
        {isVerificationPending ? (
          <div className="px-4 sm:px-6 pb-8 pt-2 flex flex-col items-center text-center animate-in fade-in zoom-in duration-300">
            <div className="bg-white border border-[#2E3344]/8 rounded-xl p-4 w-full mb-6 shadow-sm">
              <p className="text-xs font-semibold text-[#746E73] uppercase tracking-wider mb-1">Link expires in</p>
              <p className={`text-3xl font-black ${timeLeft < 60 ? 'text-red-500 animate-pulse' : 'text-[#A7653A]'}`}>
                {formatTime(timeLeft)}
              </p>
              {timeLeft === 0 && (
                <p className="text-xs text-red-500 mt-2 font-medium">Verification link expired. Please sign up again.</p>
              )}
            </div>
          ...
          <form onSubmit={handleSubmit} className="px-4 sm:px-6 pb-6 space-y-4 mt-4 animate-in fade-in duration-300">
              ...
              {isLogin && (
                <div className="flex items-center justify-between mt-2">
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    />
                    <span className="text-xs font-medium text-[#746E73]">Remember me</span>
                  </label>
                  <button
                    type="button"
            ...
            {(isLogin || (!isLogin && role === "owner")) && (
              <div className="px-4 sm:px-6 pb-6 pt-2">
                ...
                >
                  ...
                  <span className="truncate">{isLogin ? "Sign in as Shop Owner" : "Continue with Google"}</span>
                </Button>
                {isLogin && (
                  <p className="text-center text-[10px] sm:text-[11px] text-[#746E73]/70 mt-2 leading-tight">
                    Google sign-in is only available for shop owner accounts. Customers should use the email login above.
                  </p>
                )}
              </div>
            )}
