...
function classifyStatus(s: PaymentStatus): StatusKind {
  if (s === "payment_verified" || s === "cod_paid" || s === "refunded") return "success";
  if (s === "payment_failed" || s === "payment_cancelled" || s === "payment_rejected") return "failed";
  return "pending";
}
...
export default async function OrderPage({
  ...
  const needsReceipt =
    (order.payment_method === "bank_transfer" || order.payment_method === "qr_code") &&
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-2xl mx-auto p-4 sm:p-6 space-y-4">
        <Link href={`/s/${order.shop_slug}`} className="inline-flex items-center gap-1.5 text-sm font-bold text-gray-700 hover:text-gray-900">
          <ArrowLeft className="h-4 w-4" /> Back to {order.shop_name}
        </Link>

        {/* Hero status card */}
        <div className={`rounded-3xl p-6 shadow-sm border ${
          kind === "success" ? "bg-green-50 border-green-200" :
          kind === "failed"  ? "bg-red-50 border-red-200" :
                               "bg-amber-50 border-amber-200"
        }`}>
          <div className="flex items-start gap-4">
            <div className={`h-12 w-12 rounded-2xl flex items-center justify-center shrink-0 ${
              kind === "success" ? "bg-green-100" :
              kind === "failed"  ? "bg-red-100" :
                                   "bg-amber-100"
            }`}>
              {kind === "success" ? <CheckCircle2 className="h-6 w-6 text-green-700" /> :
               kind === "failed"  ? <XCircle className="h-6 w-6 text-red-700" /> :
                                    <Clock className="h-6 w-6 text-amber-700" />}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-[10px] font-black uppercase tracking-wider text-gray-500">Payment Status</p>
              <h1 className={`text-lg font-black ${
                kind === "success" ? "text-green-900" :
                kind === "failed"  ? "text-red-900" : "text-amber-900"
              }`}>
                {PAYMENT_STATUS_LABELS[order.payment_status] ?? order.payment_status}
              </h1>
              <p className="text-xs text-gray-600 mt-1">
                Method: <span className="font-bold">{PAYMENT_METHOD_LABELS[order.payment_method]}</span>
              </p>
              {order.rejected_reason && (
                <p className="mt-2 text-xs bg-white/60 rounded-lg px-3 py-2 text-red-900">
                  <span className="font-bold">Reason:</span> {order.rejected_reason}
                </p>
              )}
              {paymentParam === "failed" && reason && (
                <p className="mt-2 text-xs bg-white/60 rounded-lg px-3 py-2 text-red-900">
                  <span className="font-bold">Gateway:</span> {reason.replace(/_/g, " ")}
                </p>
              )}
            </div>
          </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            <div>
              <p className="text-[10px] font-black uppercase tracking-wider text-gray-400">Order</p>
              <p className="font-mono font-black text-gray-900">{order.order_number}</p>
            </div>
            ...
            {order.items.map((it, i) => (
              <li key={i} className="px-5 py-3 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500">Qty {it.qty} × Rs. {it.price.toLocaleString()}</p>
                </div>
                <p className="font-black text-sm text-gray-900 shrink-0">Rs. {(it.qty * it.price).toLocaleString()}</p>
              </li>
            ))}
          </ul>
          ...
          <div className="px-5 py-4 bg-gray-50 flex items-center justify-between">
            ...
            <span className="text-lg font-black text-gray-900">Rs. {Number(order.total_amount).toLocaleString()}</span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-5 space-y-2 text-sm">
          ...
          {order.customer_name  && <p><span className="text-gray-500">Name:</span> <span className="font-bold text-gray-900">{order.customer_name}</span></p>}
