...
import { ShopsSection, Shop } from "@/components/sections/ShopsSection";
...
export default function HomeClient({
  ...
  function addProductToBasket(productId: string) {
    const product = trendingProducts.find((item) => item.id === productId);
    setBasketItems((current) =>
      current.includes(productId) ? current : [...current, productId],
    );
    if (product) {
      toast.success(`${product.name} added by barcode`, {
        description: `${product.barcode} · ${product.shop_name}`,
      });
    }
  }
  ...
  return (
    <div
      ref={rootRef}
      className="quivo-award-shell min-h-screen overflow-hidden bg-[#F8FAFC] font-[Poppins] text-[#1E293B]"
    >
      ...
      <main id="top" className="pt-16 sm:pt-20">
        ...
        />
        <ShopsSection shops={activeShops} />
        <FeaturesSection />
        <ScanToOrderSection
