...
export async function startStockTake(shopId: string, notes?: string) {
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .single();
    if (error) {
      log.error("startStockTake failed", {
        code: error.code,
        message: error.message,
      });
      if (error.code === "23505")
        return { error: "A stock take is already open for this shop." };
...
export async function upsertStockTakeCount(
  input: z.infer<typeof UpsertCountSchema>,
) {
  ...
  try {
    ...
    if (error) {
      log.error("upsertStockTakeCount failed", {
        code: error.code,
        message: error.message,
      });
      return { error: error.message };
...
export async function completeStockTake(id: string) {
  ...
  try {
    ...
    if (error) {
      log.error("completeStockTake failed", {
        code: error.code,
        message: error.message,
      });
      return { error: error.message };
