...
const PromoSchema = z.object({
  shopId:       UUID,
  code:         z.string().trim().min(2).max(40).regex(/^[A-Z0-9_-]+$/i, "Letters, digits, _ and - only"),
...
export async function listPromoCodes(shopId: string): Promise<{ rows: PromoRow[]; error?: string }> {
  const parse = UUID.safeParse(shopId);
  if (!parse.success) return { rows: [], error: "Invalid shop id" };
  const supabase = await createClient();
  const { data, error } = await supabase
    .from("promo_codes")
    .select("id, code, kind, value, min_subtotal, max_discount, max_uses, used_count, valid_from, valid_to, active, created_at")
...
export async function createPromoCode(input: z.infer<typeof PromoSchema>): Promise<{ id?: string; error?: string }> {
  ...
  if (parse.data.validFrom && parse.data.validTo && new Date(parse.data.validFrom) >= new Date(parse.data.validTo)) {
  ...
  const { data, error } = await supabase
    ...
    .single();
  if (error) {
    log.warn("createPromoCode failed", { code: error.code, message: error.message });
    if (error.code === "23505") return { error: "A code with that name already exists for this shop." };
...
export async function togglePromoActive(id: string, active: boolean): Promise<{ ok?: true; error?: string }> {
  const parse = UUID.safeParse(id);
  if (!parse.success) return { error: "Invalid id" };
  const supabase = await createClient();
  const { error } = await supabase.from("promo_codes").update({ active }).eq("id", parse.data);
  if (error) return { error: "Could not update." };
  revalidatePath("/dashboard/owner/payments/promo-codes");
  return { ok: true };
}

export async function deletePromoCode(id: string): Promise<{ ok?: true; error?: string }> {
  const parse = UUID.safeParse(id);
  if (!parse.success) return { error: "Invalid id" };
  const supabase = await createClient();
  const { error } = await supabase.from("promo_codes").delete().eq("id", parse.data);
...
export async function previewPromoCode(input: z.infer<typeof ApplySchema>): Promise<{ preview?: PromoPreview; error?: string }> {
