...
import type { PaymentMethod, OwnerPaymentConfig } from "@/lib/payments";
...
export async function getOwnerPaymentConfig(
  ...
  if (!data) {
    return {
      config: {
        enabled_methods: ["cod"],
        esewa_merchant_code: null,
        esewa_environment: "sandbox",
        has_esewa_secret: false,
        khalti_public_key: null,
        khalti_environment: "sandbox",
        has_khalti_secret: false,
        bank_name: null,
...
export async function updateOwnerPaymentConfig(
  ...
  const parsed = ConfigSchema.safeParse({
    ...
    esewa_environment: formData.get("esewa_environment")?.toString() as
      | "sandbox"
      | "production"
      | undefined,
    khalti_public_key: formData.get("khalti_public_key")?.toString(),
    khalti_secret_key: formData.get("khalti_secret_key")?.toString(),
    khalti_environment: formData.get("khalti_environment")?.toString() as
      | "sandbox"
      | "production"
      | undefined,
...
export async function uploadQrCodeImage(
  ...
  const {
    data: { publicUrl },
  } = supabase.storage.from("shop_assets").getPublicUrl(path);
...
export async function verifyPayment(
  ...
  const { supabase } = await getAuthUser();
  const { error } = await supabase.rpc("verify_payment_by_owner", {
...
export async function markCodPaid(
  ...
  const { supabase } = await getAuthUser();
  const { error } = await supabase.rpc("mark_cod_paid_by_owner", {
    p_payment_id: parse.data,
  });
  if (error) return { error: error.message };
  revalidatePath("/dashboard/owner/orders");
...
export async function getReceiptSignedUrl(
  ...
  const { data, error } = await supabase.storage
    ...
    .createSignedUrl(
      pathParse.data,
      Math.max(60, Math.min(expiresInSec, 3600)),
    );
  if (error) return { error: error.message };
