...
export async function getWalletRedeemMax(shopId: string, subtotal: number): Promise<{ max: number }> {
  const parse = UUID.safeParse(shopId);
  if (!parse.success || !(subtotal > 0)) return { max: 0 };
  const supabase = await createClient();
  const { data } = await supabase
    .rpc("get_wallet_redeem_max", { p_shop_id: parse.data, p_subtotal: subtotal })
    .maybeSingle<number>();
