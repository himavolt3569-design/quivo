...
export async function getProductProfitability(
  ...
  if (!ShopIdSchema.safeParse(shopId).success)
    return { error: "Invalid shop ID" };
  if (
    !IsoSchema.safeParse(startIso).success ||
    !IsoSchema.safeParse(endIso).success
  )
  ...
  try {
    ...
    if (error) {
      log.error("getProductProfitability failed", {
        code: error.code,
...
export async function getTopProducts(
  ...
  try {
    ...
    if (error) {
      log.error("getTopProducts failed", {
        code: error.code,
        message: error.message,
      });
      return { error: error.message };
...
export async function getTopCustomers(
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .eq("shop_id", shopId)
      .order(by === "orders" ? "order_count" : "total_spent", {
        ascending: false,
      })
      .limit(Math.max(1, Math.min(limit, 200)));
    if (error) {
      log.error("getTopCustomers failed", {
        code: error.code,
        message: error.message,
      });
      return { error: error.message };
...
export async function getSalesByStaff(
  ...
  try {
    ...
    if (error) {
      log.error("getSalesByStaff failed", {
        code: error.code,
        message: error.message,
      });
      return { error: error.message };
