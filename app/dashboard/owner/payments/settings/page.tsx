...
export default async function PaymentSettingsPage() {
  ...
  if (res.error || !res.config) {
    return <div className="p-6 text-sm text-red-600">Failed to load: {res.error}</div>;
  }
    ...
    <div className="max-w-3xl mx-auto pb-12 space-y-5">
      ...
      <div>
        ...
        <p className="text-xs text-[#746E73] mt-0.5">
          Configure which payment methods <span className="font-bold">{shop.name}</span> offers.
          Each shop has its own credentials — they are never shared across shops.
        </p>
      </div>
